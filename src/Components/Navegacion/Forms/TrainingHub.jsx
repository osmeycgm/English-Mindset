...
const TrainingHub = () => {
    const { token, user, hasActivePlan, loadingUser, fetchUserProfile } = useUser()
    ...
    // ← FIX 1: useEffect que revalida el plan al entrar al Training Hub
    useEffect(() => {
        if (token) {
            fetchUserProfile()
        }
    ...
    // ← FIX 2: usa loadingUser en vez de loading (que no existe)
    if (loadingUser) {
        return (
        ...
        return (
            <div style={{
                ...
                <div style={{
                    ...
                    boxShadow: "0 20px 40px rgba(0,0,0,0.4)", padding: "3rem",
                    maxWidth: "36rem", width: "100%", textAlign: "center"
                }}>
                    <div style={{
                        ...
                        border: "1px solid rgba(56,189,248,0.2)", margin: "0 auto 1.5rem auto"
                    }}>
        ...
        <div className="aesthetic-bg py-5">
            <div className="container" style={{ maxWidth: "1200px" }}>
                    ...
                    <div className="animate__animated animate__fadeIn position-relative py-4">
                        ...
                        <div className="position-relative d-flex flex-column align-items-center gap-5 my-5">
                            <svg className="d-none d-md-block position-absolute w-100 h-100" style={{ top: 0, left: 0, pointerEvents: "none", zIndex: 0 }}>
                                <path d="M 600, 50 Q 250, 180 600, 320 T 600, 600 T 600, 880"
                                    fill="none" stroke="#38bdf8" strokeWidth="2" strokeDasharray="8,8" opacity="0.4" />
                            </svg>
                            ...
                            {nivelesAcademicos.map((nivel, idx) => {
                                ...
                                return (
                                    <div key={nivel.id}
                                        ...
                                        style={{ zIndex: 2 }}>
                                        <div className={`island-node text-center p-4 ${estaBloqueado ? "locked-node" : "active-node"} ${isExam ? "exam-node" : ""}`}
                        ...
                        <div className="d-flex align-items-center justify-content-center" style={{ minHeight: "50vh" }}>
                            <div className="glass-card text-center p-5 w-100" style={{ maxWidth: "600px", borderTop: "4px solid #f59e0b" }}>
                                ...
                                <p className="text-blue-200 mb-5">
                                    Aquí montaremos la interfaz estilo Kahoot interactivo para evaluar tu nivel exacto.
                                </p>
                                <button className="btn btn-warning fw-bold py-3 px-5 rounded-pill shadow-glow text-dark fs-5 w-100"
                            ...
                            <div className="row align-items-center position-relative" style={{ zIndex: 2 }}>
                                ...
                                <div className="col-12 col-lg-4 mt-4 mt-lg-0 text-center text-lg-end">
                                    <div className="p-3 rounded-3 bg-white bg-opacity-10 backdrop-blur" style={{ border: "1px solid rgba(255,255,255,0.15)" }}>
                                        ...
                                        <a href={linkGoogleMeet} target="_blank" rel="noopener noreferrer"
                            ...
                            <div className="position-relative">
                                ...
                                <div className="netflix-row" ref={carouselRef}>
                                    {unidadesMock.map((unidad) => (
                                        <div key={unidad.id} className="netflix-card" onClick={() => handleEntrarUnidad(unidad)}>
                                            <div className="netflix-card-img-wrapper">
                                                <img src={unidad.image} alt={unidad.title} className="netflix-card-img" loading="lazy" />
                ...
                {vistaActual === "unit-detail" && (
                    <Unit unidad={unidadSeleccionada} onVolver={() => setVistaActual("micro")} />
                )}
            ...
            <style>{`
                ...
                .btn-neon {
                    ...
                    box-shadow: 0 0 15px rgba(37, 99, 235, 0.4); transition: 0.3s;
                }
                .btn-neon:hover { transform: translateY(-2px); box-shadow: 0 0 25px rgba(56, 189, 248, 0.7); color: white; }
                .island-node {
                ...
                .active-node:hover { transform: scale(1.06) translateY(-5px); border-color: #38bdf8; box-shadow: 0 10px 30px rgba(56, 189, 248, 0.25); }
                .exam-node { border-color: rgba(245, 158, 11, 0.4); }
                .exam-node:hover { border-color: #f59e0b; box-shadow: 0 10px 30px rgba(245, 158, 11, 0.25); }
                .locked-node { opacity: 0.4; cursor: not-allowed; background: rgba(15, 23, 42, 0.8); }
                .node-badge {
                    position: absolute; top: -12px; left: 20px;
                    background: #0284c7; padding: 2px 12px; font-size: 0.75rem;
                    border-radius: 20px; font-weight: bold;
                }
                .netflix-section { position: relative; padding: 10px 0; }
                .netflix-row {
                ...
                .netflix-card {
                    flex-shrink: 0; width: 260px; height: 380px;
                    background: #1e293b; border-radius: 16px; overflow: hidden;
                    cursor: pointer; position: relative;
                    transition: all 0.4s cubic-bezier(0.25, 1, 0.5, 1);
                    box-shadow: 0 10px 20px rgba(0,0,0,0.3);
                }
                .netflix-card:hover { transform: scale(1.06) translateY(-8px); box-shadow: 0 20px 35px rgba(56, 189, 248, 0.35); z-index: 10; }
                .netflix-card-img-wrapper { width: 100%; height: 65%; position: relative; overflow: hidden; }
                .netflix-card-img { width: 100%; height: 100%; object-fit: cover; transition: transform 0.6s ease; }
                .netflix-card:hover .netflix-card-img { transform: scale(1.1); }
                .netflix-card-overlay { position: absolute; inset: 0; background: linear-gradient(to bottom, transparent 40%, #1e293b 100%); }
                .netflix-card-body { padding: 16px; height: 35%; display: flex; flex-direction: column; justify-content: space-between; }
                .netflix-card-meta { font-size: 0.75rem; color: #94a3b8; display: block; margin-top: 8px; }
                .carousel-control {
                ...
                .carousel-control:hover { background: rgba(56, 189, 248, 0.9); border-color: #38bdf8; transform: translateY(-50%) scale(1.15); box-shadow: 0 10px 30px rgba(56, 189, 248, 0.4); }
                .carousel-control.left { left: -20px; }
                .carousel-control.right { right: -20px; }
                .activity-strip { background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); border-radius: 14px; cursor: pointer; transition: 0.2s; }
                .activity-strip:hover { background: rgba(56,189,248,0.08); border-color: rgba(56,189,248,0.3); transform: translateX(4px); }
                .activity-icon-box { width: 36px; height: 36px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 1.1rem; }
                ...
                .btn-back { background: transparent; color: #94a3b8; border: none; font-weight: bold; transition: 0.2s; }
